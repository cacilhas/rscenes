...
impl MeshExt for Mesh {
    ...
    fn upload(&mut self, dynamic: bool) {
        RmodelsImpl::__upload_mesh(self, dynamic)
    }

    fn update_buffer(self, index: i32, data: &mut Vec<u8>, offset: i32) {
        RmodelsImpl::__update_mesh_buffer(self, index, data, offset)
    }

    fn unload(self) {
        RmodelsImpl::__unload_mesh(self)
    }

    fn export(self, filename: impl Display) -> bool {
        RmodelsImpl::__export_mesh(self, filename)
    }

    fn get_bounding_box(self) -> BoundingBox {
        RmodelsImpl::__get_mesh_bounding_box(self)
    }
    fn gen_tangents(&mut self) {
        RmodelsImpl::__gen_mesh_tangents(self)
    }
}
